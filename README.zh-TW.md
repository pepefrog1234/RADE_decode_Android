...
後續正式版本固定使用同一把金鑰簽署。從舊版 debug APK 切換時，需要先解除安裝，
這會刪除 App 本機資料；請先另行保存需要保留的錄音與資訊。
建置、金鑰備份及 Google 套件登記請見[簽章指南](docs/signing/README.zh-TW.md)。
...
| App 圖示設計 | [megabits.xyz](https://megabits.xyz) | — |
